...
class BartMultiModalEncoder(BartEncoder):
    ...
    def forward(self,
        ...
        # print("In Encoder: ",image_embeddings,return_dict)
...
class BartMultiModalModel(BartModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        print("IB:",image_embeddings.size())
        print("EB:",encoder_outputs.last_hidden_state.size())
        ...
        print("IA:",image_embeddings.size())
        print("EA:",encoder_outputs.last_hidden_state.size())

        outputs_concantentated = torch.concat((image_embeddings,encoder_outputs.last_hidden_state),axis=1)
        if attention_mask == None:
          attention_mask_new = torch.ones((outputs_concantentated.size()[0],outputs_concantentated.size()[1]))
        else:
          attention_mask_new = torch.ones((image_embeddings.size()[0],image_embeddings.size()[1]))
          attention_mask_new = torch.concat((attention_mask_new,attention_mask),axis=1)
